...
namespace UnityEngine.GameFoundation.CatalogManagement
{
    ...
    public class GameItemDefinition : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        [SerializeField]
        [FormerlySerializedAs("m_Categories")]
        private List<int> m_CategoryHashes = new List<int>();

        [SerializeField]
        private List<string> m_CategoryIds = new List<string>();

        /// <summary>
        /// List of Category IDs assigned to this Game Item Definition.
        /// </summary>
        protected internal List<string> categories
        {
        ...
        public CategoryDefinition[] GetCategories()
        {
            ...
            var actualCategories = new List<CategoryDefinition>(m_CategoryIds.Count);
            foreach (var categoryId in m_CategoryIds)
            {
                var category = GetCategoryDefinition(categoryId);
        ...
        /// <summary>
        /// Returns a list of Category hashes assigned to this Game Item Definition.
        /// </summary>
        /// <returns>List of Category hashes assigned to this Game Item Definition.</returns>
        protected internal List<int> GetCategoryHashes()
        {
            var categoryHashes = new List<int>(m_CategoryIds.Count);

            foreach(var category in m_CategoryIds)
            {
                categoryHashes.Add(Tools.StringToHash(category));
            }
        ...
        /// <summary>
        /// Gets a CategoryDefinition from this GameItemDefinition categories with the specified id
        /// </summary>
        /// <param name="categoryId">CategoryDefinition id of CategoryDefinition to get</param>
        /// <returns>Requested Category Definition</returns>
        protected virtual CategoryDefinition GetCategoryDefinition(string categoryId)
        {
            return GameFoundationDatabaseSettings.database.gameItemCatalog.GetCategory(categoryId);
        }
        ...
        public bool AddCategory(CategoryDefinition category)
        {
            ...
            if (m_CategoryIds.Contains(category.id))
            {
        ...
        public bool RemoveCategory(CategoryDefinition category)
        {
            ...
            return m_CategoryIds.Remove(category.id);
        }
        ...
        public bool RemoveDetailDefinition(BaseDetailDefinition detailDefinition)
        {
            ...
            if (detailDefinitionType == typeof(CurrencyDetailDefinition))
            {
                InventoryDefinition walletDef = GameFoundationDatabaseSettings.database.inventoryCatalog.GetCollectionDefinition(InventoryCatalog.k_WalletInventoryDefinitionId);
                foreach (DefaultItem item in walletDef.GetDefaultItems())
                {
                    InventoryItemDefinition defaultItemDefinition = GameFoundationDatabaseSettings.database.inventoryCatalog.GetItemDefinition(item.definitionHash);
        ...
        private void OnValidate()
        {
            ...
            // check to see if this is a v 0.2.0 data schema,
            // and if so, migrate categories to 0.3.0 data schema

            if (m_CategoryIds.Count <= 0 && m_CategoryHashes.Count > 0)
            {
                foreach (var categoryHash in m_CategoryHashes)
                {
                    // currently we only care about inventory categories; game item categories was not in use
                    var categoryId = GameFoundationDatabaseSettings.database.inventoryCatalog.GetCategory(categoryHash)?.id;

                    if (!string.IsNullOrEmpty(categoryId))
                    {
                        m_CategoryIds.Add(categoryId);
                    }

                    // if we have a category hash that doesn't have an id - then it's invalid and just goes away
                }
            }

            // if there are any category ids,
            // then we can deduce that a migration from the old hash list is not needed,
            // and we can empty out the hash list, preventing future migration attempts
