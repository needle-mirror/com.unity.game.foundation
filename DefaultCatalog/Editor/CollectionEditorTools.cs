...
namespace UnityEditor.GameFoundation.DefaultCatalog
{
    ...
    static class CollectionEditorTools
    {
        ...
        /// <summary>
        /// This method checks the given <paramref name="definitionKey"/> against a list of <paramref name="existingKeys"/>, making sure that the given
        ...
        /// </summary>
        /// <param name="definitionKey">The definition Key that needs to be checked to see if it exists yet.</param>
        /// <param name="existingKeys">The list of existing objects to check their Keys against the new definitonKey.</param>
        /// <returns>The definition Key that is not a duplicate, either the original one passed in, or one with a valid number at the end.</returns>
        ...
        /// <summary>
        /// Helper method which calls <see cref="ConvertNameToKey(string)"/> followed by <see cref="DeDuplicateNewKey(string, HashSet{string})"/>
        /// </summary>
        /// <param name="displayName">The string value entered in by the user as a display name. All types
        /// of characters are possible for this value.</param>
        /// <param name="existingKeys">The list of existing objects to check their Keys against the new definitonKey.</param>
        ...
        public static string CreateUniqueCatalogPath(string filePath)
        {
            ...
            if (insertIndex < 0)
            {
                throw new ArgumentException(string.Format("Cannot create a unique catalog path with given filepath argument, {0}. Filepath must end in .asset", filePath));
            }
        ...
        /// <summary>
        /// Helper method for removing a <see cref="TagAsset" /> from an catalog asset.
        /// </summary>
        /// <param name="tagObjectToRemove">
        /// The <see cref="TagAsset" /> object to be removed from its catalog asset.
        /// </param>
        public static void RemoveObjectFromCatalogAsset(TagAsset tagObjectToRemove)
        {
            AssetDatabase.RemoveObjectFromAsset(tagObjectToRemove);
            EditorUtility.SetDirty(tagObjectToRemove.catalog);
            AssetDatabaseUpdate();
        }
