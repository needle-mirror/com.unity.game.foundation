...
namespace UnityEditor.GameFoundation.DefaultCatalog
{
    ...
    class TagPickerEditor
    {
        ...
        readonly BaseCatalogAsset m_Catalog;
        ...
        public TagPickerEditor(BaseCatalogAsset catalog)
        {
            m_Catalog = catalog;
        }
        ...
        /// <summary>
        /// Gets all tags in the game foundation database.
        /// </summary>
        /// <returns>All tags in the game foundation database.</returns>
        protected TagAsset[] GetDatabaseTags() => 
             GameFoundationDatabaseSettings.database.tagCatalog.GetTags();
        ...
        /// currently selected for tagy selection.</param>
        ...
        /// </summary>
        /// <param name="catalogItem">The GameItemDefinition of the item that is
        /// currently selected for tag selection.</param>
                        ...
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            ...
                            if (!string.IsNullOrEmpty(m_TagSearchString) && m_TagSearchResults.Count <= 0)
                            {
                                ...
                                using (new EditorGUI.DisabledGroupScope(addButtonDisabled))
                                {
                                    ...
                                    if (!tagValid)
                                    {
                                        tooltip = "Tag name invalid. Must not contain any invalid characters.";
                                    }
                                    if (tagAlreadyAdded)
                                    {
        ...
        void CreateAndAssignTagFromSearchField(CatalogItemAsset catalogItem)
        {
            ...
            {
                Debug.LogError("Could not create tag because Game Foundation database is null");
                return;
            }
            ...
            string tagKey = CollectionEditorTools.CraftUniqueKey(m_TagSearchString, new HashSet<string>(m_Tags.Select(tag => tag.key)));

            if (CollectionEditorTools.IsValidId(tagKey))
            {
                tagKey = CollectionEditorTools.DeDuplicateNewKey(tagKey, new HashSet<string>(m_Tags.Select(tag => tag.key)));
            }

            var newTag = TagAsset.Editor_Create(tagKey);
            if (newTag != null)
            {
                AddItem(newTag);
                RefreshTags();
            ...
            ResetTagSearch(true);
            UpdateTagSuggestions();
            m_TagFilterEditor.RefreshSidebarTagFilterList(m_Tags);
        }
