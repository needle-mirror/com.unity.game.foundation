...
{
    internal class StoreCategoryAssetEditor : CollectionEditorBase<CategoryAsset>
    {
        private string m_CurrentCategoryId;

        public StoreCategoryAssetEditor(string name) : base(name)
        {
        }
        ...
        public override void RefreshItems()
        {
            ...
            {
                GameFoundationDatabaseSettings.database.storeCatalog.GetCategories(GetItems());
            }
        }
        ...
        protected override void SelectItem(CategoryAsset categoryAsset)
        {
            if (categoryAsset != null)
            {
                m_ReadableNameIdEditor = new ReadableNameIdEditor(false, new HashSet<string>(GetItems().Select(i => i.id)));
                m_CurrentCategoryId = categoryAsset.id;
            }

            base.SelectItem(categoryAsset);
        }
